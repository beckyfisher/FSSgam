...
Some transformations were applied to spread the predictors more evenly
across their range. This matters in practice. A smoother fitted to a
predictor with a long right tail has most of its data in a narrow part
of the range, so the fitted curve is determined by a handful of points
at the far end. Transforming before fitting, rather than interpreting
the curve afterwards, is the more reliable approach.
...
FSSgam is also sensitive to outliers. Here these were a small number of
very large schools of fish, unlikely to be closely linked to the habitat
variables in any case. They are set to NA rather than removed row-wise,
so that a patch reef dropped from one response is still available to the
others. Note that na.omit() is applied to each response separately in
the fitting loop below: models compared by AICc must be fitted to the
same rows, so the omission has to happen once per response, before the
model set is built.
...
It is useful to look at the distribution of the continuous predictors to
confirm they are well spread across their range following the
transformations above. The rug along the bottom of each panel marks
where the observations actually lie, which shows up clustering and gaps
that a histogram’s bins can hide. A gap matters here because a smoother
fitted across one has no data to constrain its shape there.
...
![](case-study-1_files/figure-html/predictor-plots-1.png)

### Full subsets GAMM analysis

Both abundance and biomass data were modelled using a tweedie
distribution, passed to FSSgam via a list.
...
It is always a good idea to look at the response variables and check
that they suit the distribution chosen for them. All eight are counts or
biomasses bounded below at zero with a long right tail and, for the
abundances, a spike at zero, which is what the Tweedie distribution
specified below is for.
...
![](case-study-1_files/figure-html/response-plots-1.png) The models are
then implemented via a call to gam (mgcv) with the random effect of site
included using the bs=’re’ specification. Smoothers to both square-route
surface area of the patch reef as well as depth were included in all
models (and as part of the null model) via the null.terms argument of
the full subsets gam function.
...
| Response | Model | AICc | BIC | ΔAICc | ΔBIC | wi.AICc | wi.BIC | R2 | edf |
|:---|:---|---:|---:|---:|---:|---:|---:|---:|---:|
| Herbivore.abundance | complexity | 517.579 | 532.108 | 0.000 | 0.000 | 0.545 | 0.785 | 0.599 | 6.22 |
| Herbivore.biomass | complexity | 1193.364 | 1207.947 | 0.000 | 0.000 | 0.648 | 0.814 | 0.373 | 6.20 |
| Invertivore.abundance | complexity | 609.963 | 631.032 | 0.000 | 0.000 | 0.316 | 0.386 | 0.528 | 8.58 |
| Invertivore.abundance | ZONE+complexity | 610.090 | 631.416 | 0.127 | 0.384 | 0.297 | 0.319 | 0.529 | 9.04 |
| Invertivore.abundance | SCORE2+complexity | 610.183 | 632.211 | 0.220 | 1.179 | 0.283 | 0.214 | 0.534 | 9.35 |
| Invertivore.biomass | complexity | 1236.190 | 1251.080 | 0.000 | 0.000 | 0.489 | 0.715 | 0.405 | 6.42 |
| Piscivore.abundance | ZONE+complexity | 457.410 | 473.733 | 0.000 | 0.041 | 0.241 | 0.317 | 0.316 | 6.01 |
| Piscivore.abundance | complexity | 457.424 | 473.692 | 0.014 | 0.000 | 0.239 | 0.323 | 0.320 | 5.64 |
| Piscivore.abundance | ZONE+complexity.by.ZONE | 458.610 | 475.481 | 1.199 | 1.790 | 0.132 | 0.132 | 0.299 | 7.00 |
| Piscivore.abundance | SCORE1.by.ZONE+ZONE | 458.616 | 477.036 | 1.206 | 3.344 | 0.132 | 0.061 | 0.299 | 7.86 |
| Piscivore.biomass | SCORE1 | 1023.310 | 1035.207 | 0.000 | 0.000 | 0.288 | 0.417 | 0.285 | 5.05 |
| Piscivore.biomass | complexity | 1023.701 | 1035.418 | 0.392 | 0.212 | 0.237 | 0.375 | 0.304 | 5.00 |
| Planktivore.abundance | SCORE1 | 686.785 | 702.014 | 0.000 | 0.000 | 0.453 | 0.665 | 0.332 | 6.72 |
| Planktivore.biomass | ZONE+complexity | 1011.661 | 1033.323 | 0.000 | 0.000 | 0.471 | 0.486 | 0.328 | 9.65 |
| Planktivore.biomass | complexity | 1012.084 | 1033.602 | 0.423 | 0.279 | 0.381 | 0.422 | 0.323 | 9.31 |

Top FSSgam models within ΔAICc ≤ 2, ordered best to worst within each
response. Columns follow Table A3.1 of Appendix S3. {.table}

### Relationship to the published table

The table above is recomputed each time this vignette is built, so it
will not agree digit for digit with Table A3.1 of Appendix S3, which was
produced in 2018. Comparing the two across the seventeen published rows,
AICc differed by a median of 0.66 and a maximum of 2.47 units when
measured on R 4.5.1 with `mgcv` 1.9-3 and `MuMIn` 1.48.11. Those figures
depend on the versions this site is built against and will change as
they do. The top-ranked model is the same as the published one for all
eight responses, and the R2 and edf values agree to the precision at
which they were published.

The difference is not attributable to `FSSgam`. Running the same code
against `FSSgam` 1.0.0 and against the current development version on
the same R and `mgcv` gives AICc values agreeing to within 0.02 units on
these rows, so what has changed since 2018 is `mgcv` and R, not the full
subsets procedure.

Model names are also written differently. `FSSgam` now orders the terms
within a model name in byte order rather than by the collation of the
session that happened to build the site, so the model published as
`complexity+ZONE` is named `ZONE+complexity` here. This is a naming
change only: the fitted model, its AICc, its weight and its R2 are
unaffected.
