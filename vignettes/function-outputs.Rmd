...
This vignette is based on Appendix S2 of @Fisher2018FSSgam, which documented the outputs returned by the original combined `full.subsets.gam()` function. That function has since been split into `generate_model_set()`, which builds and returns the candidate model set, and `fit_model_set()`, which fits every model in that set and returns the fitted results. The combined function itself still exists, as `full_subsets_gam()` and under its original name `full.subsets.gam()`, and returns the same combined set of outputs described here.
