...
library(dplyr)
library(tidyr)
data(coral_data, package = "FSSgam", envir = environment())
```

## Overview

Here we show some simpler examples of using FSSgam for different response distributions and particularly show the use with binomial data, which none of the original case studies were able to do.
...
`MuMIn::uGamm()` with `lme4 = TRUE` fits through `gamm4`. As of `FSSgam` 1.1.0 `gamm4`
is a suggested rather than an imported package, so installing `FSSgam` no longer installs
`gamm4` and `lme4` with it. Install them before running this section:

```{r gamm4-install, eval=FALSE}
